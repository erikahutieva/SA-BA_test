...
- [Use Case 1. Публикация товара на витрине](#use-case-1-публикация-товара-на-витрине)
  - [Название](#название-1)
  - [Актор](#актор)
  - [Предусловия](#предусловия)
  - [Основной сценарий](#основной-сценарий)
  - [Постусловия](#постусловия)
- [Use Case 2. Ошибка публикации из-за незаполненных данных](#use-case-2-ошибка-публикации-из-за-незаполненных-данных)
  - [Название](#название-2)
  - [Актор](#актор-1)
  - [Предусловия](#предусловия-1)
  - [Основной сценарий](#основной-сценарий-1)
  - [Альтернативный поток](#альтернативный-поток)
  - [Постусловия](#постусловия-1)
- [Use Case 3. Завершение модерации](#use-case-3-завершение-модерации)
  - [Название](#название-3)
  - [Актор](#актор-2)
  - [Предусловия](#предусловия-2)
  - [Основной сценарий](#основной-сценарий-2)
  - [Альтернативный сценарий](#альтернативный-сценарий)
  - [Постусловия](#постусловия-2)
...
- [Задание 3.1 — REST API](#задание-31-rest-api)
  - [Описание](#описание)
  - [HTTP-метод и URL](#http-метод-и-url)
  - [Входные параметры (тело запроса)](#входные-параметры-тело-запроса)
  - [Выходные параметры (успешный ответ, HTTP 201)](#выходные-параметры-успешный-ответ-http-201)
  - [Выходные параметры (ответ с ошибкой)](#выходные-параметры-ответ-с-ошибкой)
  - [Описание кодов ошибок](#описание-кодов-ошибок)
  - [Пример запроса](#пример-запроса)
  - [Успешный ответ (201)](#успешный-ответ-201)
  - [Ответ при слабом пароле (400)](#ответ-при-слабом-пароле-400)
  - [Ответ при существующем пользователе (409)](#ответ-при-существующем-пользователе-409)
  - [Ответ при непрохождении reCAPTCHA (400)](#ответ-при-непрохождении-recaptcha-400)
  - [Описание в формате OpenAPI](#описание-в-формате-openapi)

- [Задание 3.2 — Алгоритм создания пользователя на бэкенде](#задание-32--алгоритм-создания-пользователя-на-бэкенде)
  - [Шаг 1. Разбор тела запроса](#шаг-1-разбор-тела-запроса)
  - [Шаг 2. Проверка обязательных полей](#шаг-2-проверка-обязательных-полей)
  - [Шаг 3. Валидация форматов](#шаг-3-валидация-форматов)
  - [Шаг 4. Проверка сложности пароля](#шаг-4-проверка-сложности-пароля)
  - [Шаг 5. Проверка Google reCAPTCHA](#шаг-5-проверка-google-recaptcha)
  - [Шаг 6. Проверка уникальности пользователя](#шаг-6-проверка-уникальности-пользователя)
  - [Шаг 7. Хеширование пароля](#шаг-7-хеширование-пароля)
  - [Шаг 8. Сохранение пользователя](#шаг-8-сохранение-пользователя)
  - [Шаг 9. Обработка ошибок базы данных](#шаг-9-обработка-ошибок-базы-данных)
  - [Шаг 10. Формирование успешного ответа](#шаг-10-формирование-успешного-ответа)
  - [Описание в виде диаграммы](#описание-в-виде-диаграммы)
...
9.1. Нарушение уникального индекса по username

Если INSERT падает с ошибкой нарушения уникального ограничения (constraint violation) на поле username — это значит, что между проверкой в шаге 6 и вставкой в шаге 8 другой запрос успел создать пользователя с тем же именем (race condition). В этом случае:

- откатить транзакцию;
- вернуть HTTP 409 Conflict;
- код ошибки USER_ALREADY_EXISTS, сообщение "User existed" — то есть тот же ответ, что и в шаге 6, только обнаруженный позже, на уровне БД, а не на уровне предварительной проверки.

9.2. Любая другая ошибка БД

Недоступность БД, таймаут соединения, ошибки транзакции, не связанные с уникальным индексом:
