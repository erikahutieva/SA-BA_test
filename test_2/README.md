...
**Актор:** система B2B (без участия пользователя).

- Срабатывает синхронно в момент авто-отвязки позиции (FR-01)
- Выполняет поиск среди маркированных артикулов по партномеру, вендору, категории (FR-02)
- Создаёт задачу на разбор с результатом поиска: без кандидата / один кандидат / несколько кандидатов (FR-03)
- Не выполняет привязку самостоятельно ни при каком результате

### UC-01 · Найти отвязанные позиции поставщика

**Актор:** контент-менеджер. Дополнительный способ найти позицию, минуя очередь задач (например, для повторного разбора отклонённой задачи).
...
**Актор:** контент-менеджер. Выполняется, если по задаче на разбор кандидатов не найдено, либо кандидаты отклонены.

- Наименование и партномер предзаполняются данными позиции, доступны для правки
- После «Добавить» система создаёт и сразу связывает артикул; строка исчезает из непривязанных, связанная задача на разбор закрывается (FR-15)
...
**Актор:** контент-менеджер. Выполняется при подтверждении единственного кандидата из задачи на разбор либо при ручном подборе через UC-01.
...
- Если действие выполнено из задачи на разбор — задача закрывается (FR-15)

### UC-05 · Разобрать неоднозначный случай (несколько кандидатов)

**Актор:** контент-менеджер, при необходимости — совместно с закупками.

- Открывает задачу на разбор со списком кандидатов
- Автовыбор недоступен — решение принимается вручную (BR-07, FR-05)
- По итогу разбора выполняется либо UC-04 (привязка к одному из кандидатов), либо UC-02 (создание нового артикула), задача закрывается соответствующим результатом
...
### UC-00 · Автоматический подбор артикула-кандидата

Given маркированная позиция была привязана, When поставщик передаёт признак маркировки, Then система в той же транзакции устанавливает article_id_fk = null.
Given произошла авто-отвязка, When система обрабатывает событие, Then выполняется поиск среди ARTICLE с is_marked = true по партномеру, вендору, категории.
Given кандидатов не найдено, Then создаётся UNLINK_TASK со статусом «новая», пустым списком кандидатов и пометкой «требуется новый артикул».
Given найден ровно один кандидат с совпадением по всем трём полям, Then задача создаётся с этим кандидатом, помеченным «сильный».
Given найдено несколько кандидатов (любая комбинация сильных/слабых), Then все они попадают в задачу без предвыбора — автопривязка не выполняется.
Given любой из исходов, Then система сама не выполняет привязку ни в одном случае.
(добавить) Given поиск кандидата завершился ошибкой/таймаутом, Then ошибка логируется, а позиция остаётся доступной через ручной поиск (UC-01) — это единственное явно отсутствовавшее у тебя условие: без него неясно, что происходит при сбое.

### UC-01 · Найти отвязанные позиции

Given выбран фильтр «Поставщик», When список загружается, Then показаны только позиции этого поставщика.
Given фильтры по умолчанию, Then видны только позиции «в наличии» и «участвует в ценообразовании»; переключатель «Привязанные» выключен.
Given нажато «Показать все», Then оба базовых фильтра снимаются.
Given позиция без article_id_fk, Then она визуально отличима в списке.

### UC-02 · Создать новый артикул и привязать

Given задача без кандидатов (или кандидаты отклонены), When открыта форма создания, Then поля «Наименование» и «Партномер» предзаполнены данными поставщика и редактируемы.
When нажато «Добавить» с заполненными обязательными полями, Then артикул создаётся и сразу привязывается одной атомарной операцией.
Given операция прошла успешно, Then позиция исчезает из списка непривязанных.
Given действие выполнено из задачи на разбор, Then статус задачи меняется на «закрыта» с результатом «создан новый артикул» (FR-15).
Given артикул создан, Then признак маркировки остаётся выключенным до отдельного действия (UC-03).

### UC-03 · Включить признак маркировки

Given на артикуле есть немаркированные остатки/поставщики, When включается признак, Then сохранение блокируется с объяснением причины (BR-02).
Given конфликтов нет, When признак включён и сохранён, Then значение сохраняется как true.
Given сохранение прошло успешно, Then позиция видна на новом артикуле и отсутствует на старом.

### UC-04 · Привязать к существующему артикулу

Given выделены ровно один артикул и одна позиция, Then кнопка «Связать» активна; в противном случае — неактивна (BR-04).
Given на выбранном артикуле признак маркировки выключен, When нажато «Связать», Then действие блокируется с объяснением (BR-05).
Given выбор корректен и признак включён, When нажато «Связать», Then article_id_fk позиции обновляется на id выбранного артикула.
Given действие выполнено из задачи с единственным кандидатом, Then кандидат предзаполнен, а после привязки задача закрывается с результатом «привязано к кандидату» (FR-15).

### UC-05 · Разобрать неоднозначный случай

Given задача с 2+ кандидатами, When открыта контент-менеджером, Then все кандидаты показаны с типом совпадения (сильный/слабый), без предвыбора.
Then система не предлагает массовое подтверждение или автовыбор для такой задачи (BR-07, FR-05).
Given выбрана привязка, Then применяется поток UC-04, задача закрывается с указанием выбранного кандидата.
Given выбрано создание нового артикула, Then применяется поток UC-02, задача закрывается с результатом «создан новый артикул».
Given привлечены закупки, Then их решение можно зафиксировать (например, комментарием), но закрыть задачу может только контент-менеджер (ограничение роли из раздела 13).
